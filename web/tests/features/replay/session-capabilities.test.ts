...
describe('session capabilities', () => {
  test('keeps every V2 mode distinct and prevents non-race modes from gaining race panels', () => {
    ...
    expect(createSessionCapabilities('practice', { pitLossEstimateSidecar: {} as never }).canShowPitLoss).toBe(false)
  ...
  test('gates optional panels on the truthful mode and delivered artifacts', () => {
    ...
    const sidecarOnlyRace = createSessionCapabilities('race', { pitLossEstimateSidecar: {} as never })
    ...
    expect(sidecarOnlyRace.canShowPitLoss).toBe(true)
