 ...
 * replay-start point without current-race observedSampleCount. Catalog audit
 * metadata stays in the internal catalog; the v2 guard rejects it if smuggled
 * into the public sidecar.
 */
...
describe('parsePitLossEstimateSidecar curated baseline', () => {
  ...
  test.each([
    ['catalogVersion', 'v1'],
    ['provenance', { sourceUrl: 'https://example.com', capturedDate: '2026-03-10', evidence: 'example', method: 'official-circuit-baseline' }],
    ['evidenceCount', 1],
    ['confidence', 'high'],
    ['statusMetadata', { green: { confidence: 'high' } }],
  ])('rejects catalog-only %s metadata on a curated sidecar', (_field, value) => {
    // Arrange — catalog audit metadata must never reach the v2 public sidecar
    const payload = curatedPitLossEstimateSidecarPayload() as Record<string, unknown>
    payload[_field as string] = value

    // Act & Assert
    expect(() => parsePitLossEstimateSidecar(payload)).toThrow(`${_field} is not allowed`)
  })

  test('rejects a curated sidecar whose contract version is not v2', () => {
    // Arrange
    const payload = curatedPitLossEstimateSidecarPayload({ contractVersion: 'v1' })

    // Act & Assert
    expect(() => parsePitLossEstimateSidecar(payload)).toThrow('contract version v2')
  })
...
describe('sidecar loader integration', () => {
  ...
  test('loads a delivery without an estimate sidecar reference', async () => {
    // Arrange
    const { source } = await fixtureSourceWithSidecars({
  ...
  test('loads a digest-checked curated pit-loss estimate sidecar', async () => {
    ...
    // Assert — catalog-only audit metadata never enters the v2 sidecar and the
    // loaded object is frozen
  ...
  test('loading does not attempt to read the estimate sidecar when absent', async () => {
    // Arrange
    const { source, reads } = await fixtureSourceWithSidecars({
      omitPitLossEstimateSidecar: true,
    })
    ...
    // Assert — a pitLossModel-only delivery keeps the model and never reads the sidecar
    expect(index.pitLossEstimateSidecar).toBeUndefined()
    expect(index.manifest.pitLossEstimateSidecar).toBeUndefined()
    expect(index.pitLossModel).toBeDefined()
