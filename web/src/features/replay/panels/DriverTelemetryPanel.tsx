...
export const DriverTelemetryPanel = memo(function DriverTelemetryPanel({ drivers, selectedDriverId, snapshot }: DriverTelemetryPanelProps) {
  ...
  return (
    <article className="driver-telemetry-panel" aria-labelledby="driver-telemetry-title" style={teamAccentStyle(driver.colorHex)}>
      ...
      <TelemetryGauge sampled={sampled} />
    </article>
  )
})

function TelemetryGauge({ sampled }: { readonly sampled: ReplayDriverSnapshot | null }) {
  ...
  return (
    <div className="driver-telemetry-panel__gauge">
      <svg aria-label={accessibleLabel} className="driver-telemetry-panel__gauge-svg" role="img" viewBox="0 0 200 200">
        ...
        <g className={`driver-telemetry-panel__drs${drsValue === 'Active' ? ' driver-telemetry-panel__drs--active' : ''}`}>
          <rect height="20" rx="4" width="52" x="74" y="145" />
          <text x="100" y="159">DRS</text>
        </g>
