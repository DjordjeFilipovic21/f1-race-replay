...
def test_console_script_is_wired_to_the_cli_main() -> None:
    ...
    # Assert
    assert configuration["project"]["scripts"]["f1-replay-pipeline"] == "f1_replay_pipeline.cli:main"


def test_importing_cli_does_not_import_fastf1_or_the_canonical_writer() -> None:
    # Arrange
    sys.modules.pop("fastf1", None)
    sys.modules.pop("f1_replay_pipeline.canonical_writer", None)
    sys.modules.pop("f1_replay_pipeline.cli", None)

    # Act
    __import__("f1_replay_pipeline.cli")

    # Assert
    assert "fastf1" not in sys.modules
    assert "f1_replay_pipeline.canonical_writer" not in sys.modules
