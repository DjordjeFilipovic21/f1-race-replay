...
def test_manifest_omits_sidecar_when_unset_and_includes_when_present() -> None:
    without_sidecar = BrowserManifest("race-01", "Race", DRIVERS, session_mode="race")
    assert "pitLossEstimateSidecar" not in without_sidecar.as_dict()

    reference = _sidecar_reference()
    with_sidecar = BrowserManifest(
        "race-01", "Race", DRIVERS, session_mode="race", pit_loss_estimate_sidecar=reference,
    )
    ...
    from_mapping = BrowserManifest(
        "race-01", "Race", DRIVERS, session_mode="race", pit_loss_estimate_sidecar=reference.as_dict(),
    )
    assert from_mapping.pit_loss_estimate_sidecar == reference


def test_manifest_rejects_pit_loss_estimate_sidecar_for_non_race_sessions() -> None:
    with pytest.raises(ValueError, match="race-only browser sidecars"):
        BrowserManifest(
            "practice-01",
            "Practice",
            DRIVERS,
            session_mode="practice",
            pit_loss_estimate_sidecar=_sidecar_reference(),
        )
