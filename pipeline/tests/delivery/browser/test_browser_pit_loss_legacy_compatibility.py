"""Focused v2 coverage for the preserved ``track-status-median-v1`` method.

The v2 browser delivery publishes the global prior-weighted pit-loss model and,
optionally, the status-aware ``track-status-median-v1`` estimate sidecar.  These
tests verify that pitLossModel-only and sidecar-free deliveries remain readable
under the v2 contract, that causal timelines round-trip, that browser chunk
shapes stay byte-identical with or without the additive artifacts, and that
publication never touches templates/ or R2 paths.  ``track-status-median-v1``
is a method identifier, not a contract version: the v2 sidecar keeps the method
so existing artifacts and the web selector stay readable.
"""
...
def _track_status_median_sidecar() -> BrowserPitLossEstimateSidecar:
...
def test_pit_loss_model_only_delivery_remains_readable_and_valid(tmp_path: Path) -> None:
    # Arrange: a delivery carrying only the pit-loss model.
    ...
    # complete delivery validates under the v2 contract.
...
def test_sidecar_free_delivery_remains_readable_and_valid(tmp_path: Path) -> None:
    # Arrange / Act: publish a delivery with neither additive pit-loss artifact.
    result = _publish(tmp_path / "browser", _delivery())
    manifest = json.loads(result.manifest_path.read_bytes())

    # Assert: the v2 manifest omits both artifacts and still validates.
...
def test_pit_loss_model_and_estimate_sidecar_coexist(tmp_path: Path) -> None:
    # Arrange: the model and the track-status-median-v1 estimate sidecar coexist.
    delivery = _delivery(
        pit_loss_model=_model(), pit_loss_estimate_sidecar=_track_status_median_sidecar(),
    )
...
def test_pit_loss_artifacts_never_contain_source_status(tmp_path: Path) -> None:
    # Arrange / Act: publish the model and estimate sidecar together.
    result = _publish(
        tmp_path / "browser",
        _delivery(
            pit_loss_model=_model(), pit_loss_estimate_sidecar=_track_status_median_sidecar(),
...
def test_track_status_median_causal_timeline_round_trips_through_publication(
    tmp_path: Path,
) -> None:
    # Arrange: a multi-point causal timeline remains a supported v2 shape.
    sidecar = _track_status_median_sidecar()

    # Act: publish the sidecar and parse its stored payload.
...
def test_track_status_median_sidecar_emits_known_values_when_sc_vsc_absent() -> None:
...
def test_track_status_median_sidecar_marks_occurring_but_unobserved_status_unavailable() -> None:
...
def test_additive_pit_loss_artifacts_do_not_change_browser_chunk_shapes(tmp_path: Path) -> None:
    # Arrange: one delivery without additive artifacts and one with the model
    # and estimate sidecar.
    bare = _delivery()
    enriched = _delivery(
        pit_loss_model=_model(), pit_loss_estimate_sidecar=_track_status_median_sidecar(),
    )
...
def test_publication_never_touches_templates_or_r2_paths(tmp_path: Path) -> None:
    # Arrange: publish the richest delivery (model + estimate sidecar).
